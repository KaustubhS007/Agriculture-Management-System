...
app.config['MYSQL_PASSWORD'] = 'cricket@123'
app.config['MYSQL_DB'] = 'eagr'
...
def manage():
    if 'user' in session:
        ...
        rj= cur.execute("SELECT * FROM subsap")
...
def products():
    if 'user' in session:
        name = session['user']
        cur = mysql.connection.cursor()
        hj= cur.execute("SELECT * FROM products")
...
def access():
    if 'user' in session:
        ...
        hj=cur.execute("DELETE from subsap WHERE cid='"+cid+"'")
...
def order_placed():
    if 'user' in session:
        ...
        cost1 = int(cost)
        cur = mysql.connection.cursor()
        hj= cur.execute("INSERT INTO orders(pid,cid,cost,daddress,quantity,tcost) values(%s,%s,%s,%s,%s,%s)",(pid,cid,cost1,customer_info[0]['address'],quantity,str (int(cost)*int(quantity) )))
...
def sub_ap():
    if 'user' in session:
        ...
        customer_info = get_cid_info(name)
        cur = mysql.connection.cursor()
        hj= cur.execute("INSERT INTO subsap(name,email,address,ainc,adpf,inpf,cid) values(%s,%s,%s,%s,%s,%s,%s)",(customer_info[0]['nam'],customer_info[0]['email'],customer_info[0]['address'],ainc,data,data1,customer_info[0]['cid']))
        mysql.connection.commit()
        return redirect(url_for('index'))
...
def register_saved():
    ...
    conn.execute("INSERT INTO customer(password,address,name,phnno,email) values(%s,%s,%s,%s,%s)",(passwd,addrf,namef,phnno,email))
    mysql.connection.commit()
...
def get_cid_info(cid):
    conn = mysql.connection.cursor()
    hj= conn.execute("CALL new_p('"+str(int(cid))+"') ")
    #hj= conn.execute("select * from customer where cid='{}' ".format(cid))
...
def mlogadd():
    ...
    conn = mysql.connection.cursor()
    hj= conn.execute("SELECT uid,password,name,addr FROM admin where uid='"+email+"'")
    hj = [dict(uid=col[0],password=col[1],name=col[2],addr=col[3]) for col in conn.fetchall()]
    print(hj)
    if hj:
        #f hj[0]['uid'] == email and hj[0]['password'] == password: 
        if not(hj[0]['uid'] == email and hj[0]['password'] == password):    
            #print("alay")
